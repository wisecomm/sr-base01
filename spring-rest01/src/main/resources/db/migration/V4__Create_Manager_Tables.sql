-- 1. CHMM_USER_INFO (관리자 정보)
 CREATE TABLE CHMM_USER_INFO (
     USER_ID             VARCHAR(50) PRIMARY KEY,    -- 관리자_아이디 (기본 키)
     USER_EMAIL          VARCHAR(100) UNIQUE,        -- 관리자_이메일 (고유)
     USER_MOBILE         VARCHAR(20),                -- 관리자_모바일
     USER_NAME           VARCHAR(100) NOT NULL,      -- 관리자_명
     USER_NICK           VARCHAR(100),               -- 관리자_닉네임
     USER_PWD            VARCHAR(255) NOT NULL,      -- 관리자_암호 (해시된 비밀번호)
     USER_MSG            TEXT,                       -- 관리자_메시지
     USER_DESC           TEXT,                       -- 관리자_설명
     USER_STAT_CD        VARCHAR(10),                -- 관리자_상태_코드 (예: 'ACTIVE', 'INACTIVE')
     USER_SNSID          VARCHAR(100),               -- 관리자_SNS_아이디
     USE_YN              CHAR(1) DEFAULT '1' NOT NULL, -- 사용 여부 ('1': 사용, '0': 미사용)
     ACCOUNT_NON_LOCK    CHAR(1) DEFAULT '1' NOT NULL, -- 관리자 계정 잠김 여부 ('1': 잠금 아님, '0': 잠김)
     PASSWORD_LOCK_CNT   INTEGER DEFAULT 0 NOT NULL, -- 비밀번호 잠금 시도 횟수
     ACCOUNT_START_DT    TIMESTAMP WITHOUT TIME ZONE, -- 관리자 시작날짜
     ACCOUNT_END_DT      TIMESTAMP WITHOUT TIME ZONE, -- 관리자 종료날짜
     ...
     SYS_INSERT_USER_ID  VARCHAR(50),                -- 시스템_입력_관리자_아이디
     SYS_UPDATE_DTM      TIMESTAMP WITHOUT TIME ZONE DEFAULT CURRENT_TIMESTAMP, -- 시스템_수정_일시
     SYS_UPDATE_USER_ID  VARCHAR(50)                 -- 시스템_수정_관리자_아이디
 );
 ...
 CREATE TABLE CHMM_MENU_INFO (
     ...
     LEFT_MENU_YN        CHAR(1) DEFAULT 'Y' NOT NULL, -- 좌측 메뉴 여부 ('Y': 예, 'N': 아니오)
     USE_YN              CHAR(1) DEFAULT '1' NOT NULL, -- 사용 여부 ('1': 사용, '0': 미사용)
     ADMIN_MENU_YN       CHAR(1) DEFAULT 'N' NOT NULL, -- 어드민 메뉴 여부 ('Y': 예, 'N': 아니오)
     MENU_HELP_URI       VARCHAR(255),               -- 도움말 리소스 경로
     MENU_SCRIPT         TEXT,                       -- 메뉴 스크립트
     PERSONAL_DATA_YN    CHAR(1) DEFAULT 'N' NOT NULL, -- 개인정보 포함 여부 ('Y': 예, 'N': 아니오)
     SYS_INSERT_DTM      TIMESTAMP WITHOUT TIME ZONE DEFAULT CURRENT_TIMESTAMP, -- 시스템_입력_일시
     SYS_INSERT_USER_ID  VARCHAR(50),                -- 시스템_입력_관리자_아이디
     SYS_UPDATE_DTM      TIMESTAMP WITHOUT TIME ZONE DEFAULT CURRENT_TIMESTAMP, -- 시스템_수정_일시
     SYS_UPDATE_USER_ID  VARCHAR(50),                -- 시스템_수정_관리자_아이디
 ...
 -- 4. CHMM_USER_ROLE_MAP (관리자-역할 매핑)
 CREATE TABLE CHMM_USER_ROLE_MAP (
     USER_ID             VARCHAR(50) NOT NULL,       -- 관리자_아이디 (FK)
     ROLE_ID             VARCHAR(50) NOT NULL,       -- 롤_아이디 (FK)
     USE_YN              CHAR(1) DEFAULT '1' NOT NULL, -- 사용 여부 ('1': 사용, '0': 미사용)
     SYS_INSERT_DTM      TIMESTAMP WITHOUT TIME ZONE DEFAULT CURRENT_TIMESTAMP, -- 시스템_입력_일시
     PRIMARY KEY (USER_ID, ROLE_ID),                 -- 복합 기본 키
     FOREIGN KEY (USER_ID) REFERENCES CHMM_USER_INFO(USER_ID) ON DELETE CASCADE, -- 관리자 삭제 시 매핑 삭제
